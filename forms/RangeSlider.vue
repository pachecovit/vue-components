<script>
  export default {
    ...
    computed: {
      flowDirection () {
        return `range-slider-${this.direction + (this.reverse ? '-reverse' : '')}`
      },
      tooltipDirection () {
      ...
      tooltipStatus () {
        return this.tooltip === 'hover' && this.flag ? 'range-slider-always' : this.tooltip ? `range-slider-${this.tooltip}` : ''
      },
      tooltipClass () {
        return [`range-slider-tooltip-${this.tooltipDirection}`, 'range-slider-tooltip']
      },
      isMobile () {
      ...
      disabledClass () {
        return this.disabled ? 'range-slider-disabled' : ''
      },
      isRange () {
      ...
      total () {
        if (this.data) {
          ...
          console.error('[range-slider warn]: Prop[interval] is illegal, Please make sure that the interval can be divisible')
        }
...
<template>
  <div ref="wrap" :class="['range-slider-wrap', flowDirection, disabledClass, { 'range-slider-has-label': piecewiseLabel }]" v-show="show" :style="wrapStyles" @click="wrapClick">
    <div ref="elem" aria-hidden="true" class="range-slider" :style="[elemStyles, bgStyle]">
      <template v-if="isMobile">
        <template v-if="isRange">
          <div ref="dot0" :class="[tooltipStatus, 'range-slider-dot']" :style="[sliderStyles[0], dotStyles]" @touchstart="moveStart(0)">
            <span :class="['range-slider-tooltip-' + tooltipDirection[0], 'range-slider-tooltip-wrap']">
                              <slot name="tooltip" :value="val[0]" :index="0">
                                  <span class="range-slider-tooltip" :style="tooltipStyles[0]">{{ formatter ? formatting(val[0]) : val[0] }}</span>
            </slot>
            </span>
          </div>
          <div ref="dot1" :class="[tooltipStatus, 'range-slider-dot']" :style="[sliderStyles[1], dotStyles]" @touchstart="moveStart(1)">
            <span :class="['range-slider-tooltip-' + tooltipDirection[1], 'range-slider-tooltip-wrap']">
                              <slot name="tooltip" :value="val[1]" :index="1">
                                  <span class="range-slider-tooltip" :style="tooltipStyles[1]">{{ formatter ? formatting(val[1]) : val[1] }}</span>
            </slot>
            </span>
          </div>
        </template>
        <template v-else>
          <div ref="dot" :class="[tooltipStatus, 'range-slider-dot']" :style="[sliderStyles, dotStyles]" @touchstart="moveStart">
            <span :class="['range-slider-tooltip-' + tooltipDirection, 'range-slider-tooltip-wrap']">
      ...
      <template v-else>
        <template v-if="isRange">
          <div ref="dot0" :class="[tooltipStatus, 'range-slider-dot']" :style="[sliderStyles[0], dotStyles]" @mousedown="moveStart(0)">
            <span :class="['range-slider-tooltip-' + tooltipDirection[0], 'range-slider-tooltip-wrap']">
                              <slot name="tooltip" :value="val[0]" :index="0">
                                  <span class="range-slider-tooltip" :style="tooltipStyles[0]">{{ formatter ? formatting(val[0]) : val[0] }}</span>
            </slot>
          ...
          <div ref="dot1" :class="[tooltipStatus, 'range-slider-dot']" :style="[sliderStyles[1], dotStyles]" @mousedown="moveStart(1)">
            <span :class="['range-slider-tooltip-' + tooltipDirection[1], 'range-slider-tooltip-wrap']">
                              <slot name="tooltip" :value="val[1]" :index="1">
                                  <span class="range-slider-tooltip" :style="tooltipStyles[1]">{{ formatter ? formatting(val[1]) : val[1] }}</span>
            </slot>
        ...
        <template v-else>
          <div ref="dot" :class="[tooltipStatus, 'range-slider-dot']" :style="[sliderStyles, dotStyles]" @mousedown="moveStart">
            <span :class="['range-slider-tooltip-' + tooltipDirection, 'range-slider-tooltip-wrap']">
                              <slot name="tooltip" :value="val">
                                  <span class="range-slider-tooltip" :style="tooltipStyles">{{ formatter ? formatting(val) : val }}</span>
            </slot>
            </span>
          </div>
        </template>
      ...
      <template>
        <ul class="range-slider-piecewise">
          <li v-for="(piecewiseObj, index) in piecewiseDotWrap" :style="[piecewiseDotStyle, piecewiseObj.style]">
            <slot name="piecewiseL" :label="piecewiseObj.label" :index="index" :first="index === 0" :last="index === piecewiseDotWrap.length - 1">
              <span v-if="piecewise" class="range-slider-piecewise-dot" :style="[ piecewiseStyle, piecewiseObj.inRange ? piecewiseActiveStyle : null ]"></span>
            </slot>
            <slot name="label" :label="piecewiseObj.label" :index="index" :first="index === 0" :last="index === piecewiseDotWrap.length - 1">
              <span v-if="piecewiseLabel" class="range-slider-piecewise-label" :style="[ labelStyle, piecewiseObj.inRange ? labelActiveStyle : null ]">
                                  {{ piecewiseObj.label }}
                              </span>
            </slot>
          </li>
      ...
      <div ref="process" class="range-slider-process" :style="processStyle"></div>
    </div>
    <input v-if="!isRange && !data" class="range-slider-sr-only" type="range" v-model="val" :min="min" :max="max" />
  </div>
</template>
